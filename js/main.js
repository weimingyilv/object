...
//增加百分比
function addPercent() {
    var $xmgl = $('.xmgl').find('.percent').text(),
        $ldl = $('.ldl').find('.percent').text(),
        $ywnl = $('.ywnl').find('.percent').text(),
        $jsnl = $('.jsnl').find('.percent').text(),
        $gt = $('.gt').find('.percent').text(),
        $sjgl = $('.sjgl').find('.percent').text();
 
    if ($xmgl == xmgl) {
        clearInterval(timer);
    } else {      
        if ($ldl == ldl) {
            $('.ldl').find('.percent').text(ldl);
        } else {
            $('.ldl').find('.percent').text(i);
        }
        if ($ywnl == ywnl) {
            $('.ywnl').find('.percent').text(ywnl);
        } else {
            $('.ywnl').find('.percent').text(i);
        }
        if ($jsnl == jsnl) {
            $('.jsnl').find('.percent').text(jsnl);
        } else {
            $('.jsnl').find('.percent').text(i);
        }
        if ($gt == gt) {
            $('.gt').find('.percent').text(gt);
        } else {
            $('.gt').find('.percent').text(i);
        }
        if ($sjgl == sjgl) {
            $('.sjgl').find('.percent').text(sjgl);
        } else {
            $('.sjgl').find('.percent').text(i);
        }
        $('.xmgl').find('.percent').text(Math.floor((xmgl - $xmgl) / k) + i);
        i += 1;
        rotate();
    }
...
function section6() {
    ...
    $(document).ready(function(){
        ...
        $('#fullpage').fullpage({
            ...
            navigationTooltips: ['欢迎界面', '关于我', '专业技能', '工作经历', '作品展示', '联系方式'],
...
console.log("这是他的邮箱:%c qinjun163email@163.com","color:red");
console.log("要不打个电话试试:%c 131-1368-3440","color:red");
});//readyfunction结束
